...
def index(request):
    if request.method == 'POST':
        ...
        input_path = settings.BASE_DIR + obj.photo.url # "/media/documents/sample.jpg"
        # "/media/documents/AAA.jpeg" obj.photo.url
...
def senga(input):
    ...
    gray = imread(input, cv2.IMREAD_GRAYSCALE)
    dilated = cv2.dilate(gray, neighbourhood, iterations= 1)
    out = cv2.absdiff(dilated, gray)
    inv = 255 - out
    imwrite(settings.BASE_DIR + "/media/output/output.jpg", inv)
